...
//ctrl + c 2 раза , потом :wq! и нажать enter(пробовал ,не пошло)
//чтобы сбросить изменения git checkout/как же чистое\
// для того , чтобы создать слияние или branch , пишу эту строчку
//let a = 73; проба jshint     
//git init это 1-я команда ,инициализация в гите репозитория.Пишем из терминала репозитория,чтобы гит
//следил за этим репозиторием
//далее необходимо представиться , это имя и имейл. Пишем git config --global user.name "Vadim"
// следующее git config --global user.email vetra783@mail.ru (к примеру . Если мы вместо global 
// поставим local, то эти установки будут работать только на один репозиторий в кот. они прописаны,
//отменяя глобальные настройки.)
//Теперь добавим все файлы в репозиториии в git , пишем команду git add -A
//git add *.css ;  git add style.css; это варианты добавления файлов в гит : все файлы с расшир.css
// только style.css соответственно
//далее комент. команда : git commit -a -m"пояснение этой контрольной точки"
// далее отравляем на удалённый репозиторий git push
//git pull это команда, которя заливает изменённые данные с удалённого реп-я в ваш локальный.
//И мы можем работать,если этого не сделать ,тогда могут быть конфликты в слиянии файлов.
//Необходимо через 7 дней обновить gitkraken , чтобы оставить бесплатную версию, сейчас 
//предоставлена версия pro.        
// новый коммит)))    
//сейчас залил проэкт с удалённого репозитория .Ура!!!
//время 00.12 , залил с 2 компа на удалённый. потратил на это2 часа))
//сегодня 20.09.2020 ,добился в мал.компе в vscode прописал использование bach, на это ушла неделя,чтобы 
//разобраться , удалили все проэкты на мал. компе .Заново создал папку и залил с гит хаба.
//слова project для названия папки лучше только одно,если папка project и в ней ещё project,то 
//терминал воспринимает только корневой project ,а project в кот.происходит изменения нет.Решение 
//переименовать корневую папку.!
//моделирую слияние двух изменённых файлов
//после заливки необходимо проверять всё ли зашло, потому что лучше убедиться , что всё ок.
